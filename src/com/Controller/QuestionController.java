...
//            toIndexPage(req,resp);
    ...
    protected void ranking(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        List<Grade> grades = gradeService.queryGradeAtPaper(paper);
        request.setAttribute("grades",grades);

        System.out.println(grades);

        request.getRequestDispatcher("WEB-INF/exam/resultTable1.jsp").forward(request,response);
    }
...
//        req.getRequestDispatcher("WEB-INF/exam/paper.jsp").forward(req, resp);
    ...
    private void submitPaper(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        //返回结果集
        req.getRequestDispatcher("WEB-INF/exam/resultTable.jsp").forward(req, resp);	//通过请求转发的方式，跳转页面
    }
    //获取科目集
    private void getPaperList(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (! (ranking == null || ranking.equals(""))){
            req.setAttribute("nextPaper",ranking);
        }else{
            req.setAttribute("nextPaper","getPaperQuestion");
        }
